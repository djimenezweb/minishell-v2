...
/* 
!Problema:
Si input no es 0 quiere decir que el input viene de un archivo que hemos
abierto, así que redirigimos la entrada a ese archivo. Bien.

	if (input != STDIN_FILENO)
		dup2(input, STDIN_FILENO);

Pero el else está mal. Porque si el input es 0 no necesariamente
necesitamos un dup2(temp_fd, input). Si hay un solo comando temp_fd no
tiene ninguna utilidad, puesto que no hay ningún pipe.
